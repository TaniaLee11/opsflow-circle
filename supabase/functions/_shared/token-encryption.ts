...
import { getOAuthTokenCryptoKey } from "./oauth-token-key.ts";
...
// Key handling is centralized (strict base64 + length validation, cached import)
// in ./oauth-token-key.ts. Any misconfiguration fails fast with clear errors.
...
export async function encryptToken(plaintext: string): Promise<string> {
  ...
  const key = await getOAuthTokenCryptoKey();
  const iv = generateIV();
